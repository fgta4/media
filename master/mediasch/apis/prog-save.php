...
class DataSave extends WebAPI {
	...
	public function execute($data, $options) {
		$tablename = 'mst_mediaschprog';
		$primarykey = 'mediaschprog_id';
		...
		try {
			...
			$obj->mediaschprog_id = strtoupper($obj->mediaschprog_id);
			...
			try {
				...
				$header_table = 'mst_mediasch';
				$header_primarykey = 'mediasch_id';
			...
			$sql = \FGTA4\utils\SqlUtility::Select($tablename , [
				$primarykey,  'mediaschprog_id', 'mediaschprog_timestart', 'mediaschprog_timeend', 'mediaschprog_notes', 'mediaprog_id', 'mediasch_id', '_createby', '_createdate', '_modifyby', '_modifydate', '_createby', '_createdate', '_modifyby', '_modifydate'
			], $where->sql);
			...
			$result->dataresponse = (object) array_merge($dataresponse, [
				...
				'mediaprog_name' => \FGTA4\utils\SqlUtility::Lookup($data->mediaprog_id, $this->db, 'mst_mediaprog', 'mediaprog_id', 'mediaprog_name'),
