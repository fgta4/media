	...
	public function execute($options) {
		...
		try {
			...
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				$options->criteria,
				[
					"mediasch_id" => " mediasch_id = :mediasch_id "
				]
			);

			$sql = \FGTA4\utils\SqlUtility::Select('mst_mediasch', [
				'mediasch_id', 'mediasch_date', 'mediasch_iscommit', '_createby', '_createdate', '_modifyby', '_modifydate' 
			], $where->sql);
			...
			$result->record = array_merge($record, [
				'mediasch_date' => date("d/m/Y", strtotime($record['mediasch_date'])),
