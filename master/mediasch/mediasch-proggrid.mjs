...
const tbl_list = $('#pnl_editproggrid-tbl_list');
const txt_title = $('#pnl_editproggrid-title');
const pnl_control = $('#pnl_editproggrid-control');
const btn_removechecked  = $('#pnl_editproggrid-removechecked');
const btn_addrow = $('#pnl_editproggrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.mediasch_date)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/prog-list`
		options.criteria['id'] = data.mediasch_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editprogform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editprogform', () => {
		$ui.getPages().ITEMS['pnl_editprogform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/prog-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editprogform', ()=>{
		$ui.getPages().ITEMS['pnl_editprogform'].handler.createnew(header_data)
	})	
}