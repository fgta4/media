...
const tbl_list = $('#pnl_editslotgrid-tbl_list');
const txt_title = $('#pnl_editslotgrid-title');
const pnl_control = $('#pnl_editslotgrid-control');
const btn_removechecked  = $('#pnl_editslotgrid-removechecked');
const btn_addrow = $('#pnl_editslotgrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.mediapackage_descr)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/slot-list`
		options.criteria['id'] = data.mediapackage_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editslotform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editslotform', () => {
		$ui.getPages().ITEMS['pnl_editslotform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/slot-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editslotform', ()=>{
		$ui.getPages().ITEMS['pnl_editslotform'].handler.createnew(header_data)
	})	
}