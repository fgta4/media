...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.mediaorder_id)
	header_data = data;
}
...
function grd_list_rowrender(tr) {
	...
	$(tr).find('td').each((i, td) => {
		...
		if (mapping=='docauth_descr') {
			...
			var authby = "";
			var notes = "";			
			if (record.mediaorderappr_isdeclined=="1") {
				gambar = `<img src="images/xtiondecl.png" width="20px" height="20px">`;
				authby = `<div><span style="font-weight: bold">${record.mediaorderappr_declinedby}</span> (${record.mediaorderappr_declineddate})</div>`
				notes = record.mediaorderappr_notes.replace(/(?:\r\n|\r|\n)/g, '<br>');
				notes = `<div style="margin-top: 5px; font-style: italic">${notes}</div>`
			} else if (record.mediaorderappr_isapproved=="1") {
				gambar = `<img src="images/xtionappr.png" width="20px" height="20px">`;
				authby = `<div><span style="font-weight: bold">${record.mediaorderappr_by}</span> (${record.mediaorderappr_date})</div>`
			} else {
