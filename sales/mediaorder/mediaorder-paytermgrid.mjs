...
const tbl_list = $('#pnl_editpaytermgrid-tbl_list');
const txt_title = $('#pnl_editpaytermgrid-title');
const pnl_control = $('#pnl_editpaytermgrid-control');
const btn_removechecked  = $('#pnl_editpaytermgrid-removechecked');
const btn_addrow = $('#pnl_editpaytermgrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.mediaorder_descr)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/payterm-list`
		options.criteria['id'] = data.mediaorder_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpaytermform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpaytermform', () => {
		$ui.getPages().ITEMS['pnl_editpaytermform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/payterm-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpaytermform', ()=>{
		$ui.getPages().ITEMS['pnl_editpaytermform'].handler.createnew(header_data)
	})	
}